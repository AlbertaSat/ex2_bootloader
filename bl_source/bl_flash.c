...
bool BLInternalFlashStartAddrCheck(uint32_t ulAddr, uint32_t ulImgSize) {
    ...
    // If flashing to the golden image bank, the size of the image may not write
    // higher than 0x00200000. Which is the start of bank 1 if flashing to the
    // application image bank, the image may not write higher than 0x003FFFFF.
    // Which is the end of bank 1
...
// Bank here is not used. We calculate the bank in the function based on the
// Flash-Start-addr
